...
use App\Mail\AssetTransferCancelledMail;
use App\Models\AssetTransfer;
use Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Foundation\Queue\Queueable;
use Illuminate\Support\Facades\Mail;

class SendAssetTransferCancelledEmailJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        $transfer = AssetTransfer::query()
            ->with([
                ...
                'cancelledByUser:id,name,last_name,usuario,email',
        ...
        Mail::to(config('mail.from.address'))
            ->bcc($recipients)
            ->send(
                new AssetTransferCancelledMail(
                    $transfer,
