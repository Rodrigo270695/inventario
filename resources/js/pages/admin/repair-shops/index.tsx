...
import { RepairShopFormModal } from '@/components/organization/repair-shop-form-modal';
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Talleres externos', href: '/admin/repair-shops' },
];
...
function buildUrl(params: Partial<Filters> & { page?: number }) {
    ...
    return `/admin/repair-shops?${search.toString()}`;
}

export default function RepairShopsIndex({
    repairShops,
    zonals,
    filters,
    stats,
}: RepairShopsIndexProps) {
    ...
    const [deleteShop, setDeleteShop] = useState<RepairShop | null>(null);
    const [deleting, setDeleting] = useState(false);
    const [formOpen, setFormOpen] = useState(false);
    const [formShop, setFormShop] = useState<RepairShop | null>(null);

    const { props } = usePage();
    const auth = (props as { auth?: { permissions?: string[] } }).auth;
    const permissions = auth?.permissions ?? [];
    const canCreate = permissions.includes('repair_shops.create');
    const canUpdate = permissions.includes('repair_shops.update');
    const canDelete = permissions.includes('repair_shops.delete');
    ...
    const handleDeleteConfirm = () => {
        ...
        setDeleting(true);
        router.delete(`/admin/repair-shops/${deleteShop.id}`, {
            preserveScroll: true,
            onFinish: () => {
                setDeleting(false);
                setDeleteShop(null);
    ...
    const handleRestoreConfirm = () => {
        ...
        if (!candidate || candidate.type !== 'repair_shop' || !payload) return;
        setRestoring(true);
        router.post('/admin/repair-shops/restore', { id: candidate.id, ...payload }, {
            preserveScroll: true,
    ...
    const columns: DataTableColumn<RepairShop>[] = [
        {
            key: 'name',
            label: 'Nombre',
            sortable: true,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-3">
                        ...
                        <p className="text-muted-foreground text-sm">
                            Catálogo de talleres externos asociados a zonales.
                        </p>
                ...
                <div className="rounded-xl border border-border bg-card shadow-sm">
                    ...
                    <div className="hidden md:block">
                        <DataTable
                            ...
                            onSort={handleSort}
                            emptyMessage="No hay talleres. Crea uno con «Nuevo taller»."
                            variant="default"
                        />
                    </div>
                    <div className="md:hidden">
                        {data.length === 0 ? (
                            <p className="text-muted-foreground text-center py-8 px-4 text-sm">
                                No hay talleres. Crea uno con «Nuevo taller».
                            </p>
                        ) : (
            ...
            <DeleteConfirmModal
                ...
                title="Eliminar taller externo"
                description={
                    deleteShop
                        ? `¿Eliminar el taller «${deleteShop.name}»? Esta acción no se puede deshacer.`
                        : undefined
                }
