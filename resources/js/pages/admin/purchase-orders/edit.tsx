...
import type { BreadcrumbItem, PurchaseOrder, PurchaseItem } from '@/types';
...
/** En edición: cotización existente (id + pdf_path) o nueva (file); siempre description e is_selected editables */
type QuoteRow = {
    id?: string;
    description: string;
...
const breadcrumbs = (id: string): BreadcrumbItem[] => [
    ...
    { title: 'Editar orden', href: `/admin/purchase-orders/${id}/edit` },
];
...
function itemsToRows(items: PurchaseItem[]): ItemRow[] {
    if (!items?.length) {
        return [{
            description: '',
            quantity: 1,
    ...
    return items.map((i) => ({
        ...
        category_id: i.asset_category_id ?? '',
        asset_subcategory_id: i.asset_subcategory_id ?? '',
        asset_brand_id: i.asset_brand_id ?? '',
    }));
}
...
export default function PurchaseOrderEdit({
    ...
    const quotesInitial: QuoteRow[] = (purchaseOrder.quotes ?? []).length > 0
        ? (purchaseOrder.quotes ?? []).map((q) => ({
              id: q.id,
        ...
        : [{ description: '', file: null, is_selected: false }];

    const [data, setData] = useState({
        supplier_id: purchaseOrder.supplier_id,
        zonal_id: purchaseOrder.office?.zonal_id ?? purchaseOrder.office?.zonal?.id ?? '',
        office_id: purchaseOrder.office_id ?? purchaseOrder.office?.id ?? '',
        notes: purchaseOrder.notes ?? '',
        items: itemsToRows(purchaseOrder.items ?? []),
        quotes: quotesInitial,
    });
    ...
    const updateItem = useCallback(
        (index: number, field: keyof ItemRow, value: string | number) => {
            ...
            if (field === 'quantity' || field === 'unit_price') {
                const qty = field === 'quantity' ? (typeof value === 'number' ? value : parseNum(String(value)) || 0) : row.quantity;
                const unit = field === 'unit_price' ? (value === '' ? 0 : parseNum(String(value))) : (row.unit_price === '' ? 0 : parseNum(row.unit_price));
                row.quantity = qty;
    ...
    const addQuoteRow = useCallback(() => {
        setData((prev) => ({
            ...prev,
            quotes: [...prev.quotes, { description: '', file: null, is_selected: false }],
        }));
    }, []);

    const removeQuoteRow = useCallback((index: number) => {
        setData((prev) => {
            if (prev.quotes.length <= 1) return prev;
            return { ...prev, quotes: prev.quotes.filter((_, i) => i !== index) };
        });
    }, []);
    ...
    const updateQuote = useCallback(
        (index: number, field: keyof QuoteRow, value: string | boolean | File | null) => {
            setData((prev) => {
                ...
                row = { ...row, [field]: value } as QuoteRow;
                next[index] = row;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        data.items.forEach((row, index) => {
            ...
            formData.append(`items[${index}][quantity]`, String(Math.max(1, Number(row.quantity) || 0)));
            formData.append(`items[${index}][unit_price]`, row.unit_price === '' ? '' : String(Number(row.unit_price)));
            formData.append(`items[${index}][total_price]`, row.total_price === '' ? '' : String(Number(row.total_price)));
            formData.append(`items[${index}][category_id]`, row.category_id || '');
            formData.append(`items[${index}][asset_subcategory_id]`, row.asset_subcategory_id || '');
            formData.append(`items[${index}][asset_brand_id]`, row.asset_brand_id || '');
        });

        data.quotes.forEach((row, index) => {
            if (row.id) formData.append(`quotes[${index}][id]`, row.id);
            formData.append(`quotes[${index}][description]`, row.description);
            formData.append(`quotes[${index}][is_selected]`, row.is_selected ? '1' : '0');
            if (row.file) formData.append(`quotes[${index}][pdf]`, row.file);
        });

        setSubmitting(true);
        router.post(`/admin/purchase-orders/${purchaseOrder.id}`, formData, {
            preserveScroll: true,
            forceFormData: true,
    ...
    const subtitle = [
        purchaseOrder.code ? `Código: ${purchaseOrder.code}` : 'Sin código',
        purchaseOrder.supplier?.name ?? 'Proveedor',
    ].join(' · ');

    return (
        <AppLayout breadcrumbs={breadcrumbs(purchaseOrder.id)}>
            <Head title={`Editar orden ${purchaseOrder.code || purchaseOrder.id}`} />
            ...
            <div className="min-h-[60vh] flex flex-col gap-6 p-4 md:p-6">
                ...
                <div className="relative overflow-hidden rounded-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="relative flex flex-col gap-0">
                                ...
                                <table className="w-full border-collapse text-sm">
                                    <thead>
                                        <tr className="border-b border-border bg-muted/30">
                                            ...
                                            <th className="text-left w-44 py-3 px-2 font-medium text-muted-foreground text-xs uppercase tracking-wider">Subcategoría</th>
                                            <th className="text-left w-44 py-3 px-2 font-medium text-muted-foreground text-xs uppercase tracking-wider">Marca</th>
                                            ...
                                            <tr key={index} className="border-b border-border/60 last:border-0">
                                                ...
                                                <td className="py-2.5 px-2">
                                                    <Select
                                                        ...
                                                        onValueChange={(v) => updateItem(index, 'asset_subcategory_id', v === '_none_' ? '' : v)}
                                                ...
                                                <td className="py-2.5 px-2">
                                                    <Select
                                                        ...
                                                        onValueChange={(v) => updateItem(index, 'asset_brand_id', v === '_none_' ? '' : v)}
                                                    >
                                                        <SelectTrigger className="h-9 w-full min-w-0 rounded-xl border-border bg-background/80 shadow-xs">
                        ...
                        {/* Sección Cotizaciones (PDF) - misma estructura que create: añadir, eliminar, editar */}
                        <div className="border-t border-inv-primary/50 bg-inv-primary/25 px-4 py-3 md:px-6 dark:bg-inv-section/70 dark:border-inv-surface/60">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    >
                                        ...
                                        <div className="grid gap-3 md:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
                                            <div className="space-y-1.5">
                                                ...
                                                {quote.pdf_path && !quote.file ? (
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        <a
                                                            href={`/storage/${quote.pdf_path}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="inline-flex items-center gap-1.5 text-sm text-inv-primary hover:underline"
                                                        >
                                                            <FileText className="size-4 shrink-0" />
                                                            Ver PDF actual
                                                        </a>
                                                        <span className="text-muted-foreground text-xs">
                                                            (subir otro para reemplazar)
                                                        </span>
                                                    </div>
                                                ) : null}
                                                <input
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) =>
                                                        updateQuote(index, 'description', e.target.value)
                                                    }
