@php
    ...
    $senderName = collect([
        $transfer->sentByUser?->name,
        $transfer->sentByUser?->last_name,
    ])->filter()->join(' ') ?: ($transfer->sentByUser?->usuario ?: '—');
...
<head>
    <meta charset="UTF-8">
    <title>Traslado despachado</title>
    <style>
        ...
        .badge { display: inline-block; padding: 4px 10px; border-radius: 999px; background: #e0f2fe; color: #075985; font-size: 12px; font-weight: 700; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>Traslado despachado</h1>
            <div>Se te asignó la recepción del traslado <strong>{{ $transfer->code }}</strong>.</div>
        </div>

        <div class="content">
            <p class="meta">
                <strong>Estado:</strong> <span class="badge">En tránsito</span><br>
                <strong>Despacha:</strong> {{ $senderName }}<br>
                <strong>Recibe:</strong> {{ $receiverName }}<br>
                ...
                <strong>Fecha de envío:</strong> {{ optional($transfer->ship_date)->format('d/m/Y') ?? '—' }}
            </p>
                ...
                <tbody>
                    @forelse ($transfer->items as $index => $item)
                        <tr>
                            ...
                            <td>{{ $item->asset ? 'Activo' : ($item->component ? 'Componente' : 'Ítem') }}</td>
                            <td>{{ $conditionLabel($item->condition_out) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="muted">No se registraron ítems.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
