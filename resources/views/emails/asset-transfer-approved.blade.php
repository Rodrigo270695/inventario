@php
    ...
    $approverName = collect([
        $transfer->approvedByUser?->name,
        $transfer->approvedByUser?->last_name,
    ])->filter()->join(' ') ?: ($transfer->approvedByUser?->usuario ?: '—');
@endphp
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Traslado aprobado</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>Traslado aprobado</h1>
            <div>El traslado <strong>{{ $transfer->code }}</strong> fue aprobado y ya puedes continuar con los datos operativos.</div>
        </div>

        <div class="content">
            <p class="meta">
                <strong>Estado:</strong> <span class="badge">Aprobado</span><br>
                <strong>Solicitado por:</strong> {{ $senderName }}<br>
                <strong>Aprobado por:</strong> {{ $approverName }}<br>
                ...
                <strong>Fecha de aprobación:</strong> {{ optional($transfer->approved_at)->format('d/m/Y H:i') ?? '—' }}
            </p>
