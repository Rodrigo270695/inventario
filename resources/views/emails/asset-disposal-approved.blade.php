@php
    ...
    $creatorName = collect([
        $disposal->createdByUser?->name,
        $disposal->createdByUser?->last_name,
    ])->filter()->join(' ') ?: ($disposal->createdByUser?->usuario ?: '—');

    $approverName = collect([
        $disposal->approvedByUser?->name,
        $disposal->approvedByUser?->last_name,
    ])->filter()->join(' ') ?: ($disposal->approvedByUser?->usuario ?: '—');
@endphp
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div>Tu solicitud de baja fue aprobada.</div>
        </div>

        <div class="content">
            <p class="meta">
                <strong>Estado:</strong> <span class="badge">Aprobado</span><br>
                <strong>Solicitado por:</strong> {{ $creatorName }}<br>
                <strong>Aprobado por:</strong> {{ $approverName }}<br>
                <strong>Ubicación:</strong> {{ $path($disposal->warehouse) }}<br>
                <strong>Fecha y hora de aprobación:</strong> {{ optional($disposal->approved_at)->format('d/m/Y H:i') ?? '—' }}
            </p>
            ...
            <div class="section-title">Motivo de la baja</div>
            <div class="box">
                {{ $disposal->reason ?: '—' }}
            </div>

            <a class="button" href="{{ $detailUrl }}">Ver solicitudes de baja</a>
        </div>
    </div>
</body>
</html>
